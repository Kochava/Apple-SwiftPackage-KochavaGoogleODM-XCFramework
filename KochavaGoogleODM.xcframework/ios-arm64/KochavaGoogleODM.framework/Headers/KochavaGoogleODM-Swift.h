...
#ifndef KOCHAVAGOOGLEODM_SWIFT_H
#define KOCHAVAGOOGLEODM_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="KochavaGoogleODM",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
# pragma pop_macro("any")
#endif

#if defined(__OBJC__)

/// A feature which is responsible for a collection of conversion.
SWIFT_CLASS_NAMED("Conversion")
@interface KVAGoogleODM_Conversion : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end


@interface KVAGoogleODM_Conversion (SWIFT_EXTENSION(KochavaGoogleODM))
@end


SWIFT_CLASS_NAMED("Result")
@interface KVAGoogleODM_Conversion_Result : NSObject
...
@interface KVAConversionManager : NSObject
/// A reference to the Google AppAdsOnDeviceConversion.ConversionManager.sharedInstance.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, weak) id _Nullable shared;)
...
@class NSString;
@class KVAContext;
@class KVAGoogleODM_General;
@class KVANetworking;

/// The class GoogleODM provides an interface between a host application and Kochava’s Google ODM integration.
SWIFT_CLASS_NAMED("GoogleODM")
@interface KVAGoogleODM : NSObject
/// A shared instance, for convenience.
/// This is the preferred way of using a google odm instance.  To complete the integration you must call func <code>GoogleODM/start()</code>, or else start a controlling linked module.  You may alternatively use a constructor to create your own instance.  The shared instance simplifies your implementation as you do not need to store an instance somewhere in a public location in your own code, and you do not need to manually link together the clients of any optional products.
/// By default this instance will use the default storage location equivalent to calling <code>GoogleODM/init(storageIdentifier:)</code> with storageIdentifier nil.  If you wish to specify an alternative storage location, see var <code>sharedStorageIdentifier</code>.
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) KVAGoogleODM * _Nonnull shared;)
+ (KVAGoogleODM * _Nonnull)shared SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) KVAGoogleODM * _Nullable shared_optional;)
+ (KVAGoogleODM * _Nullable)shared_optional SWIFT_WARN_UNUSED_RESULT;
...
/// Create an instance.  You rarely create instances of class GoogleODM.  Instead you typically configure the provided shared instance and then start it using the start method.  See static var <code>shared</code>.
/// <h2>Example</h2>
/// \code
/// class MyClass
/// {
///     let googleODM = GoogleODM()
/// }
///
...
/// <h2>Example</h2>
/// \code
/// // googleODMConfigureObject
/// let googleODMConfigureObject =
/// [
///     "$override.append$":
///     [
///         "privacy":
...
///
/// // GoogleODM
/// GoogleODM.shared.configure(with: googleODMConfigureObject, context: .host)
/// GoogleODM.shared.general.registerIdentity(
///     nameString: "_SOME_NAME_",
///     valueString: "_SOME_VALUE_"
/// )
/// GoogleODM.shared.start()
///
...
/// Configure the instance with an object.
/// See func <code>GoogleODM/configure(with:context:)</code>.
...
/// Start the google odm client.
- (void)start;
/// Start the google odm client with advanced options.
/// This method can be used to start an instance of the client which was created from decoded JSON.
/// important:
...
/// Stop is similar to <code>GoogleODM/invalidate()</code> (or the use of KochavaGoogleODM.Product/reset(deleteLocalDataBool:) or KochavaGoogleODM.Product.shutdown(deleteLocalDataBool:) which wrap <code>GoogleODM/invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>GoogleODM/invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
...
- (void)stop;
/// Stop the google odm client with advanced options.
/// The primary action of this method is to reset the Networking.General.startedTask, which will cause any task which is dependent upon that task to block the initiation of new execution.  The current state of the system is fundamentally maintained;  however, essentially the same things which would not execute prior to having been started will not execute while stopped.
/// Stop is similar to <code>GoogleODM/invalidate()</code> (or the use of KochavaGoogleODM.Product/reset(deleteLocalDataBool:) or KochavaGoogleODM.Product.shutdown(deleteLocalDataBool:) which wrap <code>GoogleODM/invalidate()</code>) in that both will block new execution, though through different mechanisms;  however, it is different in that stop maintains state and can be started again.  In contrast <code>GoogleODM/invalidate()</code> discards state, cannot be re-started, and in the case of the shared instance is automatically released.
/// Stop is similar to class Networking var sleepBool in that both will block new execution through a similar mechanism;  however, it is different in that stop essentially covers all aspects of exectution which are at an inverse of start.  In contrast, class Networking var sleepBool exempts certain key features and these can continue to execute while sleep is in effect.
/// \param logMessagePrintBool A boolean indicating if a log messages consistent with this being a call to a public entry point should be printed.
...
/// Invalidate the google odm client.
...
- (void)invalidate;
/// Invalidate the google odm client with advanced options.
/// See func <code>GoogleODM/invalidate()</code>
/// \param logMessagePrintBool A boolean indicating if a log messages consistent with this being a call to a public entry point should be printed.
///
/// \param asyncBool A boolean indicating if the work within this method should asynchronously dispatch to the globalSerial queue.
///
- (void)invalidateWithLogMessagePrintBool:(BOOL)logMessagePrintBool asyncBool:(BOOL)asyncBool;
/// A feature which encapsulates all of the general aspects of a google odm instance not belonging to any other encapsulated features.
/// <h1>See</h1>
/// Class <code>GoogleODM/General</code>.
@property (nonatomic, readonly, strong) KVAGoogleODM_General * _Nonnull general;
...
@interface KVAGoogleODM (SWIFT_EXTENSION(KochavaGoogleODM)) <JSExport>
@end


@interface KVAGoogleODM (SWIFT_EXTENSION(KochavaGoogleODM))
@end


/// A feature which is responsible for a collection of datapoints.
SWIFT_CLASS_NAMED("Datapoints")
@interface KVAGoogleODM_Datapoints : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// A feature which is responsible for managing a concept of a first launch, notably including a first launch date.
SWIFT_CLASS_NAMED("FirstLaunch")
@interface KVAGoogleODM_FirstLaunch : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// A feature which encapsulates all of the general aspects of a google odm instance not belonging to any other feature components.
SWIFT_CLASS_NAMED("General")
@interface KVAGoogleODM_General : NSObject
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
@interface KochavaGoogleODM_Product : NSObject
